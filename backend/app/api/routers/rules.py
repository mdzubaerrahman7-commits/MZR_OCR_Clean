...
def exception_dashboard(
    ...
    return ExceptionDashboardOut(
        non_entitled_count=len(non_entitled),
        review_required_match_count=len(review_required),
        unknown_classification_count=len(unknown_classification),
        partial_excess_count=len([t for t in excess if t.excess_status == ExcessStatus.PARTIAL_EXCESS]),
        full_excess_count=len([t for t in excess if t.excess_status == ExcessStatus.FULL_EXCESS]),
        conversion_incomplete_count=len(conversion_incomplete),
        non_entitled_transactions=non_entitled,
        review_required_transactions=review_required,
        excess_transactions=excess,
        conversion_incomplete_transactions=conversion_incomplete,
    )
